...

{
  ...
  // hhb: N_MICRO_IN_MACRO_A = 128/8=16
  // in thread block
  // cols: micro_id_a
  // rows: micro_id_b
  ...
  /* *************************** A setup ************************** */
  /* LDS memory */
  // hhb: N_ELEMENTS_IN_PADDED_A_UNROLL = (128+1)*8=1032
 ...
 TFLOAT rC[MICRO_TILE_LENGTH_A][MICRO_TILE_LENGTH_B] = {{0.}};
  ...
  // hhb: 一个work group计算128x128个C中元素
  // 一个work group有4个wave front(64), 4x64=256个线程
  // 每个线程计算8x8个C中元素
  
  // hhb: 沿着k的方向循环展开
  // 循环展开8次，分成512/8=64次计算
  int n_unrolls_remaining = __K__ / UNROLL;
  
  while (n_unrolls_remaining > 0){
    
    /* load data from a into LDS */
      // hhb: MICRO_A_TILE_PERP_UNROLL = 4
      // MICRO_A_TILE_PLL_UNROLL = 1
      // load a -> LDS  localA
      // 需要load 128x8个A
      // 每个线程load (128x8)/256 = 4 个A
    ...
    // hhb: STRIDE_PLL_K_A = 512, UNROLL = 8  a+=512x8
    // A按列连续存储
    a += STRIDE_PLL_K_A*UNROLL;
    
    
    /* load data from b into LDS */
    // hhb: MICRO_B_TILE_PERP_UNROLL = 1
    // MICRO_B_TILE_PLL_UNROLL = 4
    // load b -> LDS localB
    // 需要load 128x8个B
    // 每个线程load (128x8)/256 = 4 个B
    ...
    // hhb: STRIDE_PLL_K_B = 1, UNROLL = 8 b+=8
    // B按列连续存储
  ...
  // hhb: N_MICRO_IN_MACRO_A = 16
  // MACRO_TILE_LENGTH_A = 128
  for (unsigned row = 0; row < MACRO_TILE_LENGTH_A; row += N_MICRO_IN_MACRO_A) {
    for (unsigned col = 0; col < MACRO_TILE_LENGTH_B; col += N_MICRO_IN_MACRO_B) {
      ...
      // hhb: STRIDE_PLL_M_C = 1, STRIDE_PLL_N_C = 512
